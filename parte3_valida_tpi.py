...
def valida_sensibilita(players: list) -> dict:
    ...
    # rinormalizzando la graduatoria non si muove quasi per costruzione. Il
    # parametro che decide chi entra in cima e' il coefficiente di ruolo, che
    # rimette in una colonna sola z calcolati dentro ruoli diversi. Quello si
    # perturba a parte, ed e' un test che puo' bocciare.
    try:
        import parte1_analisi as _p1
        rw = dict(_p1.CFG.offensive_role_weight)
    except Exception as e:
        log.warning(f"  Coefficienti di ruolo non leggibili ({e}): test parziale")
        rw = None
    if rw:
        rres = vs.role_weight_sensitivity(
            [p.get("tpi", {}).get("totale") for p in players],
            [p.get("ruolo") for p in players], rw)
        if rres:
            res["ruolo"] = rres
            q0 = rres["quota_base"]["10"]
            qmin = rres["quota_min"]["10"]
            qmax = rres["quota_max"]["10"]
            log.info(f"  SensibilitÃ  coeff. ruolo: Spearman mediana={rres['spearman_med']:.3f} "
                     f"(min {rres['spearman_min']:.3f}), overlap top10 min="
                     f"{rres['overlap_min']['10']:.2f}")
            log.info(f"    difensori in top10: base {q0.get('DIF', 0)}, "
                     f"da {qmin.get('DIF', 0)} a {qmax.get('DIF', 0)} perturbando Â±20%")
    return res
